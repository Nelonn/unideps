...
# Copies the shared libraries of the built dependencies (UNIDEPS_INSTALL_PREFIXES, set by the
# generated targets file) next to the consumer's binaries: CMAKE_RUNTIME_OUTPUT_DIRECTORY on
# Windows (defaulting to <build>/bin), CMAKE_LIBRARY_OUTPUT_DIRECTORY elsewhere (defaulting to
# <build>/lib), with a per-configuration subdirectory for multi-config generators.
macro(_unideps_copy_runtime_dependencies)
    if(WIN32)
        set(_unideps_lib_ext dll)
        if(NOT CMAKE_RUNTIME_OUTPUT_DIRECTORY)
            set(CMAKE_RUNTIME_OUTPUT_DIRECTORY "${CMAKE_BINARY_DIR}/bin")
        endif()
        set(_unideps_rt_dir "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}")
    else()
        if(APPLE)
            set(_unideps_lib_ext dylib)
        else()
            set(_unideps_lib_ext "so*")
        endif()
        if(NOT CMAKE_LIBRARY_OUTPUT_DIRECTORY)
            set(CMAKE_LIBRARY_OUTPUT_DIRECTORY "${CMAKE_BINARY_DIR}/lib")
        endif()
        set(_unideps_rt_dir "${CMAKE_LIBRARY_OUTPUT_DIRECTORY}")
    endif()

    set(_unideps_rt_dests "")
    if(CMAKE_CONFIGURATION_TYPES)
        foreach(_unideps_config IN LISTS CMAKE_CONFIGURATION_TYPES)
            list(APPEND _unideps_rt_dests "${_unideps_rt_dir}/${_unideps_config}")
        endforeach()
    else()
        list(APPEND _unideps_rt_dests "${_unideps_rt_dir}")
    endif()

    foreach(_unideps_prefix IN LISTS UNIDEPS_INSTALL_PREFIXES)
        file(GLOB_RECURSE _unideps_rt_files LIST_DIRECTORIES false "${_unideps_prefix}/*.${_unideps_lib_ext}")
        foreach(_unideps_file IN LISTS _unideps_rt_files)
            get_filename_component(_unideps_fname "${_unideps_file}" NAME)
            foreach(_unideps_dest IN LISTS _unideps_rt_dests)
                if(NOT EXISTS "${_unideps_dest}/${_unideps_fname}")
                    message(STATUS "UniDeps runtime dependency: ${_unideps_file} -> ${_unideps_dest}/${_unideps_fname}")
                endif()
                file(MAKE_DIRECTORY "${_unideps_dest}")
                file(COPY "${_unideps_file}" DESTINATION "${_unideps_dest}")
            endforeach()
        endforeach()
    endforeach()
endmacro()
...
macro(_unideps_setup_build)
    ...
    if(EXISTS "${_UNIDEPS_TARGET_FILE}")
        ...
        _unideps_copy_runtime_dependencies()
    else()
